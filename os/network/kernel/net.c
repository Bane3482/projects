...
#define PORT_QUEUE_SIZE 16

struct {
  struct spinlock lock;
  char *buf[PORT_QUEUE_SIZE];
  uint8 p;
  uint8 count;
  char init;
} portList[2049];
...
void ip_rx(char *buf, int len) {
  ...
  uint16 uport = htons(((struct udp *)(ip + 1))->dport);

  acquire(&portList[uport].lock);

  if (portList[uport].init == 0 || portList[uport].count == PORT_QUEUE_SIZE) {
